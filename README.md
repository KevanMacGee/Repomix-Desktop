...
- More error handling has been added but there are some edge cases that haven't been addressed yet. I'd move it to "not really broken anymore and only just a little bit incomplete"
- The app had the possibility of lagging on large repos but that should be fixed. I haven't been able to reproduce this. This should be fixed, please create an issue if you experience it .
...
- Speaking of logs, the app doesn't have any logging. If you get an error, screenshot the popup because that's the only time you will see it! Adding logs is next on the list.
